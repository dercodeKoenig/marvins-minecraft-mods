...
public class Ballista extends Entity {
        ...
        if (level() instanceof ServerLevel serverLevel) {
            ...
            if (controllingEntity != null) {
                ...
                if (controller == null || controller.getPosition(0).distanceTo(getPosition(0)) > 4) {
                    ...
                    float toRotateY = Mth.clamp(yRotDiff, -1f, 1f);

                    float xRotDiff = xRotTarget - xRotCurrent;
                    float toRotateX = Mth.clamp(xRotDiff, -5f, 5f);
    ...
    protected void defineSynchedData() {
        entityData.define(DRAW_PROGRESS, 0f);
        entityData.define(CONSTRUCTION_PROGRESS, 0);
        entityData.define(IS_BROKEN, false);
    }
