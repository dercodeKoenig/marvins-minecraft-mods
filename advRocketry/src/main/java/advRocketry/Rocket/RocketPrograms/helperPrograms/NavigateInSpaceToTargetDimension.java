...
public class NavigateInSpaceToTargetDimension {
    ...
    public static boolean run(EntityRocket rocket, ResourceLocation target, @Nullable ResourceLocation origin, Runnable onTargetReached) {
        ...
        double distanceForMaxSpeed = Config.INSTANCE.rocket_SpaceTravel_AU_Per_Second * 10; // we reach this speed only if we are 0.1 AU away from home / origin

        // slow down when near target or origin
        ...
        if (originDim != null) {
            nearOriginMultiplier = Math.min(1, Math.max(0, distanceToOrigin - entryDistanceOrigin) / distanceForMaxSpeed);
        }
