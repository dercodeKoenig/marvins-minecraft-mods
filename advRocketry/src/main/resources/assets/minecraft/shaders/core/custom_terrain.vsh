...
void main() {
    ...
    vec4 lightMapColor = minecraft_sample_lightmap(Sampler2, UV2) * Color;
...
vec3 upUniverseSpace = normalize(rotWorldInv * vec3(1,1,1)).xyz;
    ...
    for (int i = 0; i < LightCount; i++) {
        ...
        vertexColor3 += (lightMapColor.xyz) * max(0,(dot(L, normalUniverseSpace))+0.5);
    }

    vertexColor3 = clamp(vertexColor3, vec3(0.0), vec3(1));
vertexColor = vec4(vertexColor3, lightMapColor.a);

vertexColor = vertexColor / (1+vertexColor);
